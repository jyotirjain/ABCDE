...
const Dashboard = () => {
  ...
  const getUsersByJoiningDate = (users) => {
    const joinedOnDates = {};

    users.forEach((user) => {
      const joinDate = user.created_at; // Replace 'joiningDate' with the actual property that holds the joining date
      if (joinedOnDates[joinDate]) {
        joinedOnDates[joinDate]++;
      } else {
        joinedOnDates[joinDate] = 1;
      }
    });

    return joinedOnDates;
  };
  ...
  const getTotalInvestors = async () => {
    try {
      // axios
      //   .get(`${process.env.REACT_APP_TEST_URL}/v1/investors`)
      //   .then((res) => {
      //     const data = res.data.data.data.investment === "yes";
      ...
      //   setTotalInvestors(data.length); // Set the total number of investors in state
      // }
    } catch (error) {
      // Handle error if needed
    }
    ...
    axios
      ...
      .then((res) => {
        if (res.status === 200 && res.data?.data?.data) {
          ...
          const usersByJoiningDate = getUsersByJoiningDate(data);
          console.log("Users by Joining Date", usersByJoiningDate);
    ...
    <Box m="20px">
      ...
      >
        ...
        >
          <StatBox
            title={usersWithKyc}
            subtitle="no. of Kyc Completed Users"
            progress="0.80"
            increase="+43%"
            icon={
              <TrafficIcon
        ...
        >
          ...
          <Box height="250px" m="-20px 0 0 0">
            <LineChart isDashboard={true} userData={userData} />
          </Box>
        </Box>
        <Box
          gridColumn="span 4"
